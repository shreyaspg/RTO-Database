<?php
  ...
  $namee=$_POST['user'];
      ...
      if(!empty($_POST['user']))  
       { 
        $query = mysqli_query($con,"SELECT * FROM register where email = '$_POST[user]' AND pwd = '$_POST[pass]'") or die('username already in use please use another username' .mysql_error()); 
        $row = mysqli_fetch_array($query) or 
        die(header('location:login2.php?fail').mysql_error());
         if(!empty($row['email']) AND !empty($row['pwd'])) 
          { 
